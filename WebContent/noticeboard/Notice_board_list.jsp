<%@page import="Notice_vo.noticePageInfo"%>
...
<%
	ArrayList<noticeBoardBean> articleList=(ArrayList<noticeBoardBean>)request.getAttribute("articleList");
...
<body>
	...
	<section id="listForm">
		<span style="font-size: 30px; font-weight: bolder; text-align: center;">공 지 사 항</span>
		
<%if(session.getAttribute("login_id") != null){
	if (session.getAttribute("login_id").equals("admin")) { %>

			<h4><a href="noticeboardWriteForm.noticebo">게시판글쓰기</a></h4>
<% } 
}%>
...
<tbody>
	<tr>
		<td width = "70"><%=articleList.get(i).getNOTICE_NUM()%></td>
		<td width = "300">
			<a href="noticeboardDetail.noticebo?notice_num=<%=articleList.get(i).getNOTICE_NUM()%>&page=<%=nowPage%>">
				<%=articleList.get(i).getNOTICE_SUBJECT()%>
			</a>
		</td>
					
		<td width = "120"><%=articleList.get(i).getNOTICE_NAME() %></td>
		<td width = "100"><%=articleList.get(i).getNOTICE_DATE() %></td>
		<td width = "100"><%=articleList.get(i).getNOTICE_READCOUNT() %></td>
	</tr>
</tbody>			
...
var Pagination = {
    ...
    Click: function() {
        ...
        location.href = "noticeboardList.noticebo?page="+Pagination.page;
        Pagination.Start();
    },
